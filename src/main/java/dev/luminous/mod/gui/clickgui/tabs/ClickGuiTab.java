...
public class ClickGuiTab extends Tab {
    ...
    // 父类Tab需包含x/y/width/height/mc属性，否则需自行声明
    ...
    public void draw(DrawContext drawContext, float partialTicks, Color color) {
        ...
        // 重置渲染状态（语法正确，RenderSystem方法调用无误）
        ...
        if (drawBorder) {
            // 修复：booleanValue → booleanValue() （属性是Supplier时需调用方法）
            ...
            if (popped) {
                ...
                // 裁剪方法调用，语法无误（需确保Render2DUtil有此静态方法）
                Render2DUtil.startScissor(x, y + 14, width, (int) panelHeight);
