...
public class ColorComponents extends Component {
    ...
    // 补充父类未定义的parent/animation属性，匹配代码调用
    ...
    public boolean isVisible() {
        // 修复：visibility → visibility.getAsBoolean() （属性是Supplier时需调用方法）
    ...
    // 补充父类未定义的getHeight/setHeight方法，避免编译错误
    ...
    public void update(int offset, double mouseX, double mouseY) {
        clicked = false;
        int x = parent.getX        int x = parent.getX();
        ...
        if ((ClickGuiScreen.clicked || ClickGuiScreen.hoverClicked) && isVisible() && ch > 0) {
            if (!clicked) {
                ...
                if (GuiManager.currentGrabbed == null && isVisible()) {
                    // 修复：booleanValue → booleanValue()
                    if (hover && getColorSetting().injectBoolean.booleanValue()) {
    ...
    // 补充父类未定义的getTextOffsetY方法，避免编译错误
    public int getTextOffsetY() {
        return 4; // 适配GUI文本居中，可根据实际调整
    }
    ...
    public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
        ...
        // 修复：maxFill → maxFill.booleanValue()
        Render2DUtil.drawRound(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2,
                (float) defaultHeight - (ClickGui.INSTANCE.maxFill.booleanValue() ? 0 : 1),
        ...
        // 修复：injectBoolean → injectBoolean.booleanValue()
        if (colorSetting.injectBoolean.booleanValue()) {
            currentWidth = animation.get(colorSetting.booleanValue ? (width - 2D) : 0D);
            switch (ClickGui.INSTANCE.uiType.getValue()) {
                case Old -> {
                    // 修复：mainEnd → mainEnd.booleanValue()
