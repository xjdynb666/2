...
public class ColorComponents extends Component {
    ...
    private static final float PICKER_RADIUS = 6.0f;  // 颜色选择器圆角
    ...
    public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
        ...
        // 背景区域 - 使用圆角
        Render2DUtil.drawRound(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), BUTTON_RADIUS, hover ? ClickGui.INSTANCE.settingHover.getValue() : ClickGui.INSTANCE.setting.getValue());
            ...
            switch (ClickGui.INSTANCE.uiType.getValue()) {
                case Old -> {
                    if (ClickGui.INSTANCE.mainEnd.booleanValue) {
                        ...
                        Render2DUtil.drawRound(matrixStack, (float) x + 1, (float) y + 1, (float) currentWidth, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), BUTTON_RADIUS, hover ? ClickGui.INSTANCE.mainHover.getValue() : color);
                    }
                    if (unShift) {
        ...
        // 颜色预览圆形
        Render2DUtil.drawCircle(matrixStack, (float) (x + width - 10), (float) (y + getTextOffsetY() + 4), 6, ColorUtil.injectAlpha(getColorSetting().getValue(), 255));
