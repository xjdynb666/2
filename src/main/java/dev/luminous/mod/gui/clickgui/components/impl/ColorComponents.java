...
public class ColorComponents extends Component {
    ...
    public boolean isVisible() {
        return colorSetting.visibility == null || colorSetting.visibility;
    }
    ...
    public void update(int offset, double mouseX, double mouseY) {
        ...
        if ((ClickGuiScreen.clicked || ClickGuiScreen.hoverClicked) && isVisible() && ch > 0) {
            if (!clicked) {
                ...
                if (GuiManager.currentGrabbed == null && isVisible()) {
                    if (hover && getColorSetting().injectBoolean.getValue()) {
    ...
    public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
        ...
        if (colorSetting.injectBoolean.getValue()) {
                ...
                case Old -> {
                    if (ClickGui.INSTANCE.mainEnd.getValue()) {
                        Render2DUtil.drawRectHorizontal(
                                ...
                                ClickGui.INSTANCE.mainEnd.getValue()
                        );
                    } else {
                        Render2DUtil.drawRound(
                                matrixStack,
                ...
                case New -> {
                    if (unShift) {
                        TextUtil.drawString(
                                ...
                                x + 4,
                                (float) (y + getTextOffsetY()),
                                colorSetting.booleanValue
                                        ? ClickGui.INSTANCE.enableTextS.getValue()
