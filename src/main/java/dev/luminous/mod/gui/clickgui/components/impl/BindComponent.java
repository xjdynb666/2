...
public class BindComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		// 背景区域 - 使用圆角
		if (hover)
			Render2DUtil.drawRound(matrixStack, (float) parentX + 1, (float) y + 1, (float) width - 3, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), BUTTON_RADIUS, ClickGui.INSTANCE.settingHover.getValue());
