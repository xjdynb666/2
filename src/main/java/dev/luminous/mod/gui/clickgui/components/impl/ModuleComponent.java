...
public class ModuleComponent extends Component {
	...
	// 圆角半径配置
	private static final float MODULE_RADIUS = 4.0f;  // 模块列表项圆角
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (module.isOff() || !ClickGui.INSTANCE.activeBox.getValue()) {
			Color bgColor = hovered ? ClickGui.INSTANCE.moduleHover.getValue() : ClickGui.INSTANCE.module.getValue();
			Render2DUtil.drawRound(matrixStack, parentX + 1, (int) (parentY + currentOffset), parentWidth - 2, defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), MODULE_RADIUS, bgColor);
		}
		...
		if (ClickGui.INSTANCE.activeBox.getValue()) {
			if (ClickGui.INSTANCE.mainEnd.booleanValue) {
				...
				Render2DUtil.drawRound(matrixStack, parentX + 1, (int) (parentY + currentOffset), (float) currentWidth, defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), MODULE_RADIUS, hovered ? ClickGui.INSTANCE.mainHover.getValue() : ClickGui.INSTANCE.color.getValue());
			}
		}
